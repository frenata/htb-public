...
def make_traversal_path(path, level=0, os="unix", prefix=""):
    if os == "win":
        traversal = ".." + "\\"
        fullpath = prefix + traversal*level + path
...
def main():
    ...
    parser.add_argument("-r", "--prefix", dest="prefix", default="",
                        help="Prefix the levels.")
    ...
    if ext in [".zip", ".jar", ".war", ".apk"]:
        ...
        for i in range(start, end):
            fullpath = make_traversal_path(path, level=i, os=args.os, prefix=args.prefix)
